...
진행 상태(stage status)·파이프라인 개념은 없다. 세부 작업은 순서·선행조건 없이 자유 선택된다. 세부 작업이 "실행됨"인지는 `links`에 키가 존재하는지로만 판단하며, 별도 상태값을 두지 않는다. Notion 링크·메타데이터는 로컬(work.json), 산출물 본문은 Notion이 SOT.
...
상태 파일 스키마, 모든 세부작업 키와 라벨 매핑(`SUBTASK_LIST`/`SUBTASK_GROUPS`/`SUBTASK_LABELS`) — 전부 `references/state-schema.md`에 있다. TITLE_TO_KEY/KEY_TO_TITLE 런타임 정의는 `hooks/lib/constants.json` 참조. skill 본문에 중복 정의 금지.
